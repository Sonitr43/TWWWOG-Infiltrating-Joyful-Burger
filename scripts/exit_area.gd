...
@onready var exit_tutorial: CanvasLayer = get_tree().root.find_child("ExitTutorial", true, false)
...
# Funció que s'executa cada frame.
	if is_player_close and Input.is_action_just_pressed("move_up") and !hasEntered and player.is_on_floor():
	# Si el jugador està a prop, està al terra i pulsa el botó d'amunt:
		...
		# Si el node de la porta comença amb "Tutorial" (estem en un ExitArea del tutorial):
				item_list = exit_tutorial.find_child("ItemList", true, false)
				exit_tutorial.visible = true
				item_list.set_item_disabled(0, false)
		...
		# Sinó:
			...
			# Sinó (és un ExitArea d'un nivell qualsevol):
